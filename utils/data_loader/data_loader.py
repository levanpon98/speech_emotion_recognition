...
class SpeechCorpus(DataLoader):
    ...
    def __init__(self, config, is_train, is_shuffle):
        ...
        self.is_train = is_train
        self.data_dir = self.config.data_dir
        self.train_csv = self.config.train_csv
        self.batch_size = self.config.batch_size
        self.n_classes = self.config.n_classes
    ...
    def load_data(self):
        labels, mfcc_file = [], []
        with open(self.train_csv) as csv_file:
            reader = csv.DictReader(csv_file, delimiter=',')
            for row in reader:
                mfcc_file.append(self.data_dir + row['File'] + '.npy')
                labels.append(int(row['Label']))
        mfcc = []
        for file in mfcc_file:
            data = np.load(file)
            mfcc.append(data)
    ...
    def generate_train_batch(self):
        ...
        image = tf.convert_to_tensor(self.x_train, np.float32)
        label = tf.convert_to_tensor(self.y_train, np.int32)
    ...
    def generate_test_batch(self):
        ...
        image = tf.convert_to_tensor(self.x_test, np.float32)
        label = tf.convert_to_tensor(self.y_test, np.int32)
