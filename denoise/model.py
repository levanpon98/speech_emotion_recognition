...
def lossnet(input, n_layers=14, training=True, reuse=False, norm_type="SBN", ksz=3, base_channels=32, blk_channels=5):
    ...
    for id in range(n_layers):
        ...
        if id == 0:
            net = tf.nn.conv2d(input, n_channels, [1, ksz], activation_fn=leakey_relu, normalizer_fn=norm_fn,
                               stride=[1, 2],
                               scope='loss_conv_%d' % id, padding='SAME', reuse=reuse)
            layers.append(net)
        elif id < n_layers - 1:
            net = tf.nn.conv2d(layers[-1], n_channels, [1, ksz], activation_fn=leakey_relu, normalizer_fn=norm_fn,
...
def senet(input, n_layers=13, training=True, reuse=False, norm_type="NM", ksz=3, n_channels=32):
    ...
    for id in range(n_layers):
        ...
        if id == 0:
            net = tf.nn.conv2d(input, n_channels, [1, ksz], activation_fn=leakey_relu,
            ...
            net = tf.nn.conv2d(net, n_channels, [1, ksz], activation_fn=leakey_relu,
                               normalizer_fn=nm, scope='se_conv_%d' % id,
                               padding='SAME', reuse=reuse)
        ...
        output = tf.nn.conv2d(net, 1, [1, 1], activation_fn=None,
