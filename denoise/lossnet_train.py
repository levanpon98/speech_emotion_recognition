...
n_tasks += 1
asc_datasets, asc_labels, asc_names, asc_label_list = load_asc_data("dataset/asc/")
...
n_tasks += 1
dat_datasets, dat_labels, dat_names, dat_label_list = load_dat_data("dataset/dat/")
...
for id in range(n_tasks):
    ...
    opt_task.append(tf.train.AdamOptimizer(learning_rate=1e-4).minimize(loss_task[id], var_list=[var for var in
...
for epoch in range(1, Nepoch + 1):
    ...
    for id in tqdm(range(MAX_NUM_FILE * n_tasks), file=sys.stdout):
        ...
        # CLIP LABEL
        labelData = np.reshape(1. * np.array([(l in labels[ntask]['train'][file_id]) for l in label_lists[ntask]]),
                               (1, -1))
    ...
    for ntask in range(n_tasks):
        ...
        if error_type[ntask] == 1:  # MEAN CLASSIFICATION ERROR
            str += "%.6f " % (np.mean(1.0 * (np.argmax(np.vstack(train_pred_all[ntask]), axis=1) == np.argmax(
                np.vstack(train_label_all[ntask]), axis=1))))
            ...
            for nl, label in enumerate(label_lists[ntask]):
                ...
                while abs(fp - fn) > 1e-4 and abs(np.diff(thres[:-1])) > 1e-10:
                    ...
                    fp = (np.sum((np.vstack(train_pred_all[ntask])[:, nl] > thres[-1]) * (
                            1. - np.vstack(train_label_all[ntask])[:, nl]))) \
                         / (1e-15 + np.sum(1. - np.vstack(train_label_all[ntask])[:, nl]))
                    fn = (np.sum(
                        (np.vstack(train_pred_all[ntask])[:, nl] <= thres[-1]) * np.vstack(train_label_all[ntask])[:,
                                                                                 nl])) \
    ...
    for ntask in range(n_tasks):
        ...
        for id in tqdm(range(len(names[ntask]['val'])), file=sys.stdout):
            ...
            labelData = np.reshape(1. * np.array([(l in labels[ntask]['val'][file_id]) for l in label_lists[ntask]]),
                                   (1, -1))

            # PREDICTION COMPUTATION
            pred, current = sess.run([pred_task[ntask], loss_task[ntask]],
                                     feed_dict={input: inputData, label_task[ntask]: labelData})
    ...
    for ntask in range(n_tasks):
        ...
        if error_type[ntask] == 1:  # CLASSIFICATION ERROR
            str += "%.6f " % (np.mean(1.0 * (np.argmax(np.vstack(test_pred_all[ntask]), axis=1) == np.argmax(
                np.vstack(test_label_all[ntask]), axis=1))))
            ...
            for nl, label in enumerate(label_lists[ntask]):
                ...
                while abs(fp - fn) > 1e-4 and abs(np.diff(thres[:-1])) > 1e-10:
                    thres[-1] = np.mean(thres[:-1])
                    fp = (np.sum((np.vstack(test_pred_all[ntask])[:, nl] > thres[-1]) * (
                            1. - np.vstack(test_label_all[ntask])[:, nl]))) / (
                                 1e-15 + np.sum(1. - np.vstack(test_label_all[ntask])[:, nl]))
                    fn = (np.sum(
                        (np.vstack(test_pred_all[ntask])[:, nl] <= thres[-1]) * np.vstack(test_label_all[ntask])[:,
                                                                                nl])) / (
                                 1e-15 + np.sum(np.vstack(test_label_all[ntask])[:, nl]))
                    if fp < fn:
                        thres[1] = thres[-1]
                    else:
                        thres[0] = thres[-1]
